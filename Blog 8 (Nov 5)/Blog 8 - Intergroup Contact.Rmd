...
```{r}
#rowwise = creates table of functions of mutate() and summarise() 

#mutate = creates new columns can be created based on calculations or transformations of existing columns.
  #IMPORTANT for mutate: this adds the three new columns (Japan_affect, China_Affect, Affect_Diff) to it — all the original columns are still there. so make sure to only print those three columns

  #summarise() function is used for table --> can print min, max, quartiles, mean, median, std etc, 
#c in mean function means to combine

ChinaJapan <- ChinaJapan |>
  rowwise() |>
  mutate(
    Japan_affect = mean(c(JapanPos, JapanWarm, JapanAdmire, JapanRespect)), na.rm = TRUE)|>
  mutate(
    China_affect = mean(c(ChinaPos, ChinaWarm, ChinaAdmire, ChinaRespect)), na.rm = TRUE)|>
  mutate(
    Affect_diff = mean(c(PosDiff, WarmDiff, AdmireDiff, RespectDiff)), na.rm = TRUE)


print(ChinaJapan)

summary(ChinaJapan |>
  select(Japan_affect, China_affect, Affect_diff))
```
...
```{r}
#Reporting the mean values for the treatment and control groups: 

ChinaJapan |> 
  group_by(treated) |> 
  summarise(meanJapanAffect = mean(Japan_affect),
            meanChinaAffect = mean(China_affect),
            meanAffectDiff = mean(Affect_diff), 
            meanChinaPos = mean(ChinaPos)) 
```
```{r}
#test for statistical significance: we can use difference in means 

#difference_in_means(outcome ~ group, data = dataset_name)

difference_in_means(Japan_affect ~ treated, data = ChinaJapan)

difference_in_means(China_affect ~ treated, data = ChinaJapan)

difference_in_means(Affect_diff ~ treated, data = ChinaJapan)
...
```{r}
#average values for the treatment and control groups + their difference: 

ChinaJapan |> 
  group_by(treated) |> 
  summarise(meanChinaID_avg = mean(ChinaID_avg),
            meanJapanID_avg = mean(JapanID_avg),
            meanID_diff_avg = mean(ID_diff_avg)) 

```
```{r}
#difference in means for each treatment condition (essentially t-tests): 
...
```{r}
#average for each
JapanPos_avg = mean(ChinaJapan$JapanPos, na.rm = TRUE)
JapanWarm_avg = mean(ChinaJapan$JapanWarm, na.rm = TRUE)
JapanAdmire_avg = mean(ChinaJapan$JapanAdmire, na.rm = TRUE)
JapanRespect_avg = mean(ChinaJapan$JapanRespect, na.rm = TRUE)
...
JapanAffectDiff <- difference_in_means(Japan_affect ~ treated, data = ChinaJapan)

#create vectors for the plot data: 
outcomes <- c('Affect Index', 'Respect', 'Admiration', 'Warmth', 'Positivity')
pointests <- c(JapanAffectDiff$coefficients, JapanRespectDiff$coefficients, 
               JapanAdmireDiff$coefficients, JapanWarmDiff$coefficients,
               JapanPosDiff$coefficients)
lowbounds <- c(JapanAffectDiff$conf.low, JapanRespectDiff$conf.low, 
               JapanAdmireDiff$conf.low, JapanWarmDiff$conf.low,
               JapanPosDiff$conf.low)
upbounds <- c(JapanAffectDiff$conf.high, JapanRespectDiff$conf.high, 
              JapanAdmireDiff$conf.high, JapanWarmDiff$conf.high,
              JapanPosDiff$conf.high)

# Create data frame and plot
treatment_effect_plot_data <- tibble(outcomes, pointests, lowbounds, upbounds)

ggplot(treatment_effect_plot_data, 
       mapping = aes(x = factor(outcomes, levels = outcomes), 
                     y = pointests, 
                     ymin = lowbounds, 
                     ymax = upbounds)) +
  geom_pointrange() +
  ...
  coord_flip() +
  xlab('Affective Feelings towards Japanese People') +
...
* File Name: \texttt{Nov6ClassData.csv}
...
\texttt{year}  | Year in college from 1 to 4 (4+ years are indicated as 4)
...
ClassExperiment |> 
  group_by(Treated) |>
  summarise(
    mean_inparty = mean(InPartyAffect_avg, na.rm=TRUE),
    mean_outparty = mean(OutPartyAffect_avg, na.rm = TRUE), 
    mean_diff = mean(AffectDiff_avg, na.rm = TRUE), 
    mean_DemoAffect3 = mean(DemocraticAffect_3, na.rm = TRUE)
  )
```
```{r}
difference_in_means(InPartyAffect_avg ~ Treated, data = ClassExperiment)
difference_in_means(OutPartyAffect_avg ~ Treated, data = ClassExperiment)
difference_in_means(AffectDiff_avg ~ Treated, data = ClassExperiment)
difference_in_means(DemocraticAffect_3 ~ Treated, data = ClassExperiment)
```
...
ClassExperiment |> 
  group_by(Treated) |>
  summarise(
    mean_inpartyID = mean(InPartyIdentity_avg, na.rm=TRUE),
    mean_outpartyID = mean(OutPartyIdentity_avg, na.rm = TRUE), 
    mean_diffID = mean(IdentityDiff_avg, na.rm = TRUE), 
    mean_DemoID3 = mean(DemocraticIdentity_3, na.rm = TRUE)
  )
```
```{r}
difference_in_means(InPartyIdentity_avg ~ Treated, data = ClassExperiment)
difference_in_means(OutPartyIdentity_avg ~ Treated, data = ClassExperiment)
difference_in_means(IdentityDiff_avg ~ Treated, data = ClassExperiment)
difference_in_means(DemocraticIdentity_3 ~ Treated, data = ClassExperiment)
```
