...
treatment variable: day_running (continuous variable)

```{r}
protest_df$group_favorability_the_police |> table()
protest_df$discrimination_blacks |> table()
...
```{r, error=TRUE}
#Change the object to be the date of the protest spread,
#remember to put it in quotes if you copy/paste!

#making binary treatment variable 
unique(protest_df$day[which(protest_df$day_running == 0)])

protest_df_bydate <- protest_df |> 
  mutate(before = ifelse(day<as.Date("2020-05-28"), 1,0)
         protest_occured = ifelse(day<as.Date("2020-05-28"),0,1))

protest_df_bydate$before |> table()

#93966 people took the servey before that day, 284541 people took the survey after the day. 
```
...
```{r}
difference_in_means(group_favorability_the_police ~ before, 
                    data = protest_df_bydate) |> summary()

difference_in_means(discrimination_blacks ~ before, 
                    data = protest_df_bydate) |> summary()

difference_in_means(group_favorability_the_police ~ protest_occured, 
                    data = protest_df_bydate) |> summary()

difference_in_means(discrimination_blacks ~ protest_occured, 
                    data = protest_df_bydate) |> summary()

protest_df$discrimination_blacks |> table ()

#statistically significant vs substantively significant 
304k --> a small difference = a statistical significance 
```
...
```{r}
difference_in_means(group_favorability_evangelicals ~ before, 
                    data = protest_df_bydate) |> summary()
```
...
### Part a
The panels in Figure 2 in Reny and Newman (2021) graph the averages of outcome variables (`group_favorability_the_police`, `discrimination_blacks`) per day. The x axis in Figure 2 is "Days". What is the variable in our data `RN_2001_data` that matches the "Days" variable on the x axis? In order to make plots similar to Figure 2, we first have to create data that contain averages of outcome variables for each day. **Create a new data frame (or tibble) by first grouping the raw data by the variable that represent "Days" (use `group_by()`) and by calculating the average of each of the two outcome variables for each day. Be sure to preserve an indicator for whether the observations are from before or after the spread of the protests.**

```{r}
(plot_data <- protest_df_bydate |> 
  group_by(day) |> 
  summarize(mean_discrimation = mean(discrimination_blacks, na.rm = T), 
            mean_police_unfavorability = mean(group_favorability_the_police, na.rm = T), 
            before = before))
```
...
```{r}
(plot_data |> 
  ggplot(aes(x = day, y = mean_discrimation))+ 
  geom_plot() +
  geom_vline(xintercept = as.Date("2020-05-28"), color = "red", linetype = "dashed"))
```
...
```{r}
#An example of how to do multiple lies of best fit using example data from mtcars 
#(mtcars is a dataset included with the tidyverse library)
...
```{r}
plot_data |> 
  ggplot(aes(x = day, y = mean_discrimation, shape = as.factor(before)))+ 
  geom_plot()+
  geom_vline(xintercept = as.Date("2020-05-28"), color = "red", linetype = "dashed")+
  geom_smooth(method = "lm", color = "dodgerblue")

#cleaner way to look at this stuff 
```
